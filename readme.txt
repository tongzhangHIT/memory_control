...
当前参数：每隔600s检测一次dwm.exe内存占用情况，超过1024MB后关闭程序
	      程序运行情况log输出到“日期.log”文件中，可自动删除7日之前log文件
...
Current parameters: Check the memory usage of dwm.exe every 600s, and close the program when it exceeds 1024MB
		The program operation log is output to the "date.log" file, and the log file before 7 days can be automatically deleted
